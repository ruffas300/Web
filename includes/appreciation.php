...
class Appreciation {
    ...
    public static function find_for_livefeed($num_to_show=10) {
        //run sql statement through instantiate method
        global $database;
        $sql = "SELECT id, receiver_id, giver_id, date_approved, category_id, title, description FROM appreciation WHERE status_id=4 AND is_public=1 ORDER BY date_approved DESC LIMIT {$num_to_show}";
        $result_set = $database->query($sql);
        $object_array = array();
        while ($row = $database->fetch_array($result_set)) {
            $object_array[] = static::instantiate($row);
    ...
    public function create() {
        global $database;
        $sql = "INSERT INTO appreciation (receiver_id, receiver_history_id, giver_id, giver_history_id, date_given, category_id, title, description, point_value, paid_out, is_public, status_id) VALUES ('";
